...
namespace KudaGo.Application.CommandHandlers
{
    ...
    public class StartCommandHandler : ICommandHandler
    {
        ...
        {
            await _botClient.SendTypingActionAsync(messageContext.Update.Chat.Id, ct);

            var userExists = await _userRepository.UserExistsAsync(messageContext.Update.Chat.Id);
            if (userExists)
            {
